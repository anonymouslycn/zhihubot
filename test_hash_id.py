...
def getheaders():
    z1 = s.get('https://www.zhihu.com/', verify=False)
...
#==============================================================================================
    ...
    # c.set('_zap', '9f057151-7136-4d70-9fb6-9494135472a2', path='/', domain='.zhihu.com')  
    # s.cookies.update(c)   
    # c.set('cap_id', 'MjAxZjExNjBjOGE0NGI5NWEzYWExNDg1Y2I1YmZjNGI=|1518568841|1f7d63d58954fb03d5ac094e61b2273ce3517e40', path='/', domain='.zhihu.com')  
    # s.cookies.update(c)  
    # c.set('capsion_ticket', '2|1:0|10:1518570024|14:capsion_ticket|44:OTM2ZDBjZDAyZmJhNGFmMDk2MDFmYjFmODhiYjBhMjk=|e5a5f4ec3f71d94d96d18c623fb4f321ff576cfd7fb095d6434d50a98a049beb', path='/', domain='.zhihu.com')  
    # s.cookies.update(c) 
    # c.set('d_c0', 'AADtDAUZJA2PTm6j4jO7r9mjWJJgUb-tkpY=|1518540777', path='/', domain='.zhihu.com')  
    # s.cookies.update(c) 
    # c.set('l_cap_id', 'ODA3YzM4ZmRiNzYyNGQ4M2E0NmI3OWMxZjhiMmY5Yzc=|1518568841|ad9ff2d51cbcc5e56d125d3a88cc84bf3413f583', path='/', domain='.zhihu.com')  
    # s.cookies.update(c) 
    # c.set('q_c1', '2905091780694d12b061724513599354|1518539972000|1518539972000', path='/', domain='.zhihu.com')  
    # s.cookies.update(c)     
    # c.set('r_cap_id', 'ZDFkNGY1NmY4NWZhNGIwM2FkZDU1YWVhZGZiZGUzMmQ=|1518568841|a6daa8be41331e2f51b64220a0a693557b224480', path='/', domain='.zhihu.com')  
    # s.cookies.update(c)  
...
def test():
    ...
    # print (q)
    headers = getheaders()
    #知乎wap端还没有加入search_hash_id
    url2 = 'https://www.zhihu.com/r/search?'+q+'&correction=1&type=content&offset=0'  
    z2 = s.get(url2, headers=headers  )

    html = gzip.GzipFile(fileobj=io.StringIO(str(z2)),mode="r")
    ...
    tim = soup.find_all('div',class_='content')
...
##############################################################
    ...
    for i in range(len(conts)):
        temp_dict = {}
        soups = BeautifulSoup(str(conts[i]), "lxml")
        ...
        f.writelines("\n")
        f.writelines("评论时间：")
        mat = re.search(r"(\d{4}-\d{1,2}-\d{1,2})",tim[i].get_text())
        if mat != None:
            timn = mat.group()
            f.writelines(timn)
        else:
            mat = re.search(r"昨天",tim[i].get_text())
            if mat != None:
                timn = time.strftime('%Y-%m-%d',time.localtime(time.time()-86400))
                f.writelines(timn)
            else:
                timn = time.strftime('%Y-%m-%d',time.localtime(time.time()))
                f.writelines(timn)
        f.writelines("\n")
        f.writelines("评论标题：")
        f.writelines(tit[i].a.get_text())
        f.writelines("\n")
        dd = dr.sub("",cont[i].get_text())
        f.writelines("评论内容：")
        f.writelines(dd)
        f.writelines("\n\n") 
        data = [aut,timn,tit[i].a.get_text(),re.sub(r'\s+',' ', dd)] 
        writer.writerow(data)
        temp_dict['author'] = aut  
        temp_dict['date'] = timn  
        temp_dict['comment'] = re.sub(r'\s+',' ', dd) 
...
#                                                     \_/__/ 
#
#       time.sleep(random.randint(5,25))是为了避免刷新过快设置的挂起函数
        ...
        # print (len(tit),len(cont),len(tim),len(aut))
    ...
    f.writelines(html)
    f.writelines("\n")
    f.writelines(json_obj["paging"]["next"])
    f.close()
    with open(submit, 'w') as f:  
        json.dump(result, f)  
    # print (json_obj["paging"]["next"])
...
#--------------------------------------------------------------------------------------------
    ...
    try:
      ...
      print ("已加载登陆信息")
      print ("s.cookies.loads",s.cookies)
